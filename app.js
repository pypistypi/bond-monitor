...
async function loadAllData() {
    showLoader();
    try {
        // 1. Load quantities from data.json
        const qtyRes = await fetch('data.json');
        if (qtyRes.ok) {
...
async function fetchBondData(isin) {
    try {
        ...
        // 2. Find the primary board
        const boards = descJson.boards.data;
        const boardsCols = descJson.boards.columns;
        // Priority: is_primary=1, then TQCB/TQOB, then first available
        let boardRow = boards.find(row => row[boardsCols.indexOf('is_primary')] === 1);
        if (!boardRow) {
            boardRow = boards.find(row => ['TQCB', 'TQOB', 'TQOY'].includes(row[boardsCols.indexOf('boardid')]));
        }
