...
function fixCurrency(curr) {
    if (!curr) return 'RUB';
    if (curr === 'SUR') return 'RUB';
    return curr;
}

async function fetchBondData(isin) {
    try {
        // 1. Get Security Description
        ...
        // 2. Find the primary board for this security
        const boards = descJson.boards.data;
        const boardsCols = descJson.boards.columns;
        const primaryBoardRow = boards.find(row => row[boardsCols.indexOf('is_primary')] === 1) || boards[0];
        const board = primaryBoardRow[boardsCols.indexOf('boardid')];
...
function showLoader() {
    appContent.innerHTML = `
        <div class="loader-wrapper">
            ...
            <p>Загрузка данных с Московской биржи...</p>
        </div>`;
}
